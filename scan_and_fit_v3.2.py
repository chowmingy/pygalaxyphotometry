...
class galaxy:
    def __init__(self, ypos, xpos, radius):  
        self.ypos = ypos
        self.xpos = xpos
        self.radius = radius
        
    def get_info(self):
        """ Return x, y position, radius"""
        return np.array([self.ypos, self.xpos, self.radius])
    
class analysis:
    def __init__(self, fname):
        self.fname = fname
        self.raw_image_data = fits.getdata(self.fname, ext=0)###[3820:3975,422:570]   # Testing region by unccommentating
        ...
        self.rank = np.argsort(self.image_data)[::-1]  # the corresponding position of the ranking
    ...
    def rank_yx(self, rankyx, rank_to_yx=1):
        ...
        if rank_to_yx == 0:  # that means transform yx to rank, expecting rankyx to be a list
        ...
        for trial in range(self.dimension):
            ...
            if max_count >= 0:
                ...
                #         local_bg = 3419
                    ...
                    self.galaxies.append(galaxy(y, x, r_ap))
        ...
        for trial in range(self.dimension):
            ...
            for r in range(4, 80, 4):     # r = radius, we know the largest radius can't be >500 as it will be 1/4 of the pic.
                ...
                if fitted == 1 or r == 80:
                    ...
                    self.mask_region(ypos, xpos, r - 4)

                    if r - 4 > 4:               # Too small so assume counting error
                        self.galaxies.append(galaxy(ypos, xpos, r - 4))

                    else:   # r = 8
                        if no_cut < circle_number/2 and circle_number >24:
                            self.galaxies.append(galaxy(ypos, xpos, r -4, np.array(point).sum(), bg, circle_number))
                    print("\nscan of galaxy completed at radius", r - 4, "position =", ypos, xpos, "max_count =", max_count, "time = ", time.time() - start_time)
                    break
...
# ana.scan_ap(cut_off = 4200, r_ap = 12)
# fname_sc = "scan_ap_cutoff4200_v3"
...
# np.savetxt(fname_sc, info, header = "ypos xpos radius")
